...
void MyOpenGL::paintGL()
{
    ...
    vao.bind();
    glDrawElements(GL_LINES, controller_->getSizeEdges() * 2, GL_UNSIGNED_INT, 0);
    vao.release();
...
void MyOpenGL::LoadScene(QString nameFile)
{
    ...
    controller_->LoadVertics(nameFile);
    
    vertexCount = controller_->getSizeVertices();

    initVbaVbo();

    modelRadius = controller_->getRadius();
    cameraDistance = modelRadius * 3.0f;
    modelCenter = controller_->getCenter();
...
{
    vbo.bind();
    vbo.allocate(controller_->getVertices(), controller_->getSizeVertices() * 3 * sizeof(float));
    vbo.release();
    ...
    ebo.bind();
    ebo.allocate(controller_->getEdges(), controller_->getSizeEdges() * 2 * sizeof(unsigned int));
