...
class RBtree {
 protected:
  ...
  class RBTreeIteratorBase {
   ...
   public:
    ...
    void prev() {
      if (current == tree->getmin(tree->root)) {
        current = tree->nil;
      } else if (current == tree->nil) {
        current = tree->getmax(tree->root);
  ...
  class RBTreeConstIterator
      : public RBTreeIteratorBase<const Node*, const RBtree<Key, T>*> {
 ...
 public:
  ...
  std::pair<iterator, bool> insert(const value_type& value,
                                   const bool check_uniqe = true);
  std::pair<iterator, bool> insert(const key_type& k, const mapped_type& m,
                                   const bool check_uniqe = true);
  void erase(iterator pos);
  void merge(RBtree<Key, T>& other, const bool check_uniqe = true);
  void swap(RBtree<Key, T>& other);
  size_type size() { return this->size_tree; }
  size_type max_size() {
    return std::numeric_limits<size_type>::max() /
           sizeof(typename RBtree<Key, T>::Node) / 2;
  }
 ...
 protected:
  struct Node {
    ...
    Node(const key_type x = key_type(), mapped_type y = mapped_type(),
         Node* ibnl = nullptr, Node* ibnr = nullptr, Node* ub = nullptr,
         Colors col = RED)
        : date({x, y}) {
      ubi = ub;
      ibnShimala = ibnl;
...
template <typename Key, typename T>
inline typename RBtree<Key, T>::iterator RBtree<Key, T>::find(
    const key_type& k) {
  Node* node = root;
  while (node != nil) {
...
template <typename Key, typename T>
inline const typename RBtree<Key, T>::Node* RBtree<Key, T>::getmax(
...
template <typename Key, typename T>
inline const typename RBtree<Key, T>::Node* RBtree<Key, T>::getmin(
    const Node* node) const {
  if (node == nil) return nil;
  while (node->ibnShimala != nil) {
    node = node->ibnShimala;
...
template <typename Key, typename T>
inline std::pair<typename RBtree<Key, T>::iterator, bool>
RBtree<Key, T>::insert(const value_type& value, const bool check_uniqe) {
  ...
  if (root == nil) {
    ...
    if (it.current != nil && check_uniqe == true) {
      return {it, false};
    }
...
template <typename Key, typename T>
inline std::pair<typename RBtree<Key, T>::iterator, bool>
RBtree<Key, T>::insert(const key_type& k, const mapped_type& m,
                       const bool check_uniqe) {
  return insert({k, m}, check_uniqe);
}
...
template <typename Key, typename T>
inline typename RBtree<Key, T>::Node* RBtree<Key, T>::insert_node(
    Node* node, key_type k, mapped_type val) {
  Node* temp = nullptr;
  while (node != nil) {
    temp = node;
...
inline void RBtree<Key, T>::merge(RBtree<Key, T>& other,
  ...
  while (it != other.end()) {
    auto del_it = it++;
    if (this->insert(del_it->first, del_it->second, check_uniqe).second) {
      other.erase(del_it);
    }
  }
}
